...
This document describes the needed installation and configuration of supporting software for iHRIS on Ubuntu 11.04, Natty.
...
APC
^^^
We need to install the APC module for PHP.  However it is not behaving well, so we need to use Chris Lea's version
